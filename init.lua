...
do
  ...
  vim.o.number = false -- No line numbers
  vim.o.relativenumber = false -- No relative numbers
  vim.o.signcolumn = 'no' -- No gutter (sidebar)
...
do
  ...
  require('zen-mode').setup { window = { width = 1, options = { number = false, relativenumber = false, signcolumn = 'no' } } }
  ...
  require('supermaven-nvim').setup { keymaps = { accept_suggestion = '<Tab>', clear_suggestion = '<C-]>', accept_word = '<C-j>' } }
...
do
  ...
  for _, mod in ipairs { 'telescope.utils', 'plenary.utils' } do
    ...
    if ok and type(m) == 'table' and not m.if_nil then m.if_nil = vim.F.if_nil end
  end
...
do
  ...
  require('blink.cmp').setup {
    ...
    completion = {
      ...
      -- Display a preview of the documentation for the selected item
      documentation = { auto_show = true, auto_show_delay_ms = 500 },
      -- Display icons next to the completion items
